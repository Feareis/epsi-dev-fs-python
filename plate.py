...
def random_plate_2p(nb_line, nb_column, iratio=None, dratio=None):
    # Ratios aléatoires pour les murs indestructibles et cassables si non fournis
    iratio = iratio or random.uniform(0, 0.2)
    dratio = dratio or random.uniform(0, 0.4)
    ...
    if 0 <= x < nb_line and 0 <= y < nb_column:
        plate[x][y] = " "  # Position initiale du joueur 1
        # Zone joueur 1
        if y + 1 < nb_column:
            plate[x][y + 1] = " "
        if x + 1 < nb_line:
        ...
        if x + 1 < nb_line and y + 1 < nb_column:
            plate[x + 1][y + 1] = " "
    elif 0 <= x2 < nb_line and 0 <= y2 < nb_column:
        plate[x2][y2] = " "  # Position initiale du joueur 2
        # Zone joueur 2
        if y2 + 1 < nb_column:
            plate[x2][y2 + 1] = " "
        if x2 + 1 < nb_line:
            plate[x2 + 1][y2] = " "
        if x2 + 1 < nb_line and y2 + 1 < nb_column:
            plate[x2 + 1][y2 + 1] = " "
